...
const SYS = '너는 한일 통역기다. 두 사람이 마주 앉아 친해지려는 대화를 통역한다. 실제 사람이 말하듯 자연스럽고 따뜻한 구어체로, 교과서체·과한 격식·딱딱함은 피하고 친근하지만 무례하지 않게 옮긴다. 원문의 뉘앙스·감정·농담을 살린다. 지시한 형식만 정확히 출력하고 다른 말/따옴표/번호는 절대 붙이지 않는다.';
...
function buildPrompt(text, dir){
  ...
  return '다음 일본어를 한국어로 자연스럽게 통역하고, 그 한국어를 일본인이 소리 내어 읽을 수 있게 가타카나로 적어라.\n'+
